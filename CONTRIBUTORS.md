...
- [x] Merge 所有 PR（按順序：A → B → C → D → E，解決 conflict）
...
- [x] 在 `index.html` header 下方加字數統計（「已輸入 X 字」）
- [x] 在 `user-input` 的 `oninput` 事件更新字數
- [x] 在 `style.css` 加上字數統計的樣式
- [x] 填寫完整 PR 描述
- [x] 收到 2 位成員 review 後回應 comment
- [x] Review 其他組員的 PR（至少 2 個）

### 組員 D（`feature/member-d` branch）
- [x] 在 header 加深色模式切換按鈕（`🌙`）
- [x] 在 `style.css` 加上 `body.dark` 相關樣式
- [x] 在 `<script>` 加上切換函數
- [x] 填寫完整 PR 描述
- [x] 收到 2 位成員 review 後回應 comment
- [x] Review 其他組員的 PR（至少 2 個）
...
- [x] 在 `<footer>` 加入鍵盤快捷鍵說明（Enter 送出 / Esc 清空）
- [x] 在 `<script>` 加入 Esc 清空輸入框的監聽
- [x] 在 `style.css` 補 footer 樣式
- [x] 填寫完整 PR 描述
- [x] 收到 2 位成員 review 後回應 comment
- [x] Review 其他組員的 PR（至少 2 個）
